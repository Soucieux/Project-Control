...
The current source release is **v0.5 (build 5)**, delivered directly at this project root. Review fixes preserve ASCII-arrow diagrams and current sections under versioned project titles, refresh app metadata without restarting, and detect executable-permission changes. All 57 core checks, 14 store checks, native type-checking, clean compilation, strict signature/metadata/icon checks, and project-root process launch passed. The reviewed source was committed before producing this bundle, as required by the repository build-history gate.
...
The latest completed application lives directly beside this README as **Project Control.app**, now v0.5/build 5 with Nexus. The previous v0.4/build-4 bundle is preserved in `build/previous.VKEA2N/Project Control.app`; its executable, metadata, and icon checksums are unchanged. The superseded v0.3 bundle was moved to macOS Trash under `Project Control - retired v0.3 build.wLynl6`, with unchanged executable and metadata checksums. The older v0.2 and v0.1 bundles were retired to Trash during earlier updates. Quit the running app before installing an update so reopening starts the new version.
